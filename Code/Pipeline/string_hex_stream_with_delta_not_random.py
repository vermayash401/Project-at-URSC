...
def generate_month_frames(days=1, max_changes=6, seed=None):
    ...
    for i in range(total_frames):
        ...
        if i < n_same:
            mode = "low_var"# keep this as "same" if you want 33.3% same frames
...
def generate_master_correlated_frames(days=1, max_changes=6, seed=None):
    rng = random.Random(seed)
    total_frames = days * 24 * 60 * FRAMES_PER_MINUTE
    frames = []
    last_progress_len = 0
...
def generate_telemetry(days, max_changes,mode="Sequential", txt_output="spacecraft_framesX.txt"): #, bin_output= "spacecraft_framesX_delta.bin",):
    print("------------------------------------------------------------------------")
    if mode == "Master Correlated":
        frames = generate_master_correlated_frames(days, max_changes)
    else:
        frames = generate_month_frames(days, max_changes)
    #print(frames[0], "\n", frames[1])
